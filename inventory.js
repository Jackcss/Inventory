...
history.addEventListener("click",() =>{
  linkCont.style.visibility = "visible";
})
...
const saveTohistory = (products) => {
  localStorage.setItem("product_history", JSON.stringify(products));
};
...
submit_product.addEventListener("click", () => {
  ...
  const addTohistory = {
    ...
    product_cost: parseFloat(product_cost.value),
    product_quantity: number,
    product_img: imgSrc || "/images/images (1).jpeg",
  }
...
function displayProduct(product) {
  ...
  perCost.textContent = `Cost per Product: NGN${product.cost}`;

  const totalCost = document.createElement("p");
  totalCost.textContent = `Total Cost: NGN${product.cost * product.quantity}`;
  ...
  // If product already sold out, show the sold-out image and disable buttons
  ...
  // Handle product sold increment/decrement
  ...
  updateBtn.addEventListener("click", () => {
    ...
    totalCost.textContent = `Total Cost: NGN${product.cost * remaining}`;
    ...
    // Reset sold count after updating
